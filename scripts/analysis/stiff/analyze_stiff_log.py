#!/usr/bin/env python3
"""Analyze stiffness log snapshots and plot selected joint stiffness traces."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Parse a stiffness log file and plot moving-average stiffness for the requested joint IDs."
        )
    )
    parser.add_argument(
        "--log-path",
        ...
        help="Path to the stiffness log produced by actions_multi.py.",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Moving-average window size in snapshots (use 1 to plot the raw mean).",
    )
    parser.add_argument(
        "--joint-id",
        type=int,
...
def _parse_snapshot(text: str) -> np.ndarray:
    cleaned = text.replace("[", " ").replace("]", " ")
    values = np.fromstring(cleaned, sep=" ")
    if values.size == 0:
        raise ValueError("Empty stiffness snapshot encountered.")

    row_count = max(text.count("[") - 1, 1)
    if values.size % row_count != 0:
        raise ValueError(
            f"Snapshot values ({values.size}) do not divide evenly into {row_count} rows."
        )
...
def _iter_entries(lines: Iterable[str]) -> Iterable[Optional[np.ndarray]]:
    ...
    for line in lines:
        ...
        if bracket_balance == 0:
            text = "".join(buffer)
            try:
                arr = _parse_snapshot(text)
            except Exception as exc:
                raise ValueError(f"Failed to parse stiffness snapshot: {exc}\n{text}") from exc
            yield arr
            buffer.clear()
    ...
    if buffer:
        text = "".join(buffer)
        raise ValueError(
            "Trailing partial stiffness entry at end of file; ensure writes complete successfully."
        )
        ...
        if expected_size is None:
            ...
            for jid in joint_ids:
                ...
                if jid >= expected_size:
                    raise ValueError(
                        f"Joint ID {jid} is out of range for stiffness matrices of size {expected_size}."
                    )
        elif entry.shape[0] != expected_size:
            raise ValueError(
                "Encountered stiffness matrices with differing sizes; ensure the log comes from a single configuration."
            )

        diag = np.diag(entry)
        for jid in joint_ids:
            joint_series[jid].append(float(diag[jid]))
...
def plot_joint_series(
    ...
    plt.title("Stiffness statistics over time (log scale)")
